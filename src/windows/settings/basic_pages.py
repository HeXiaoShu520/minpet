...
from qfluentwidgets import InfoBar, InfoBarPosition, PushSettingCard, SettingCardGroup
from qfluentwidgets import FluentIcon as FIF

import config
from base_page import MiniPetScrollPage
from pet.codex_export import CODEX_PET_DIR, export_all_pets_to_codex
...
class AgentPage(MiniPetScrollPage):
    ...
    def __init__(self, parent=None):
        ...
        self.backendCard = ComboSettingCard(self.BACKENDS, FIF.ROBOT, '智能体模式', '选择桌宠输入、语音和投喂交给谁处理', self.agentGroup)
        self.backendCard.setCurrentValue(cfg.get('agent_backend', 'builtin'))
        self.openclawApiCard = LineEditSettingCard(FIF.LINK, 'OpenClaw Gateway 地址', 'MiniPet 直接请求这个 HTTP Responses API 地址', placeholder=config.OPENCLAW_API_URL_DEFAULT, parent=self.agentGroup)
        self.openclawApiCard.setText(cfg.get('openclaw_api_url', config.OPENCLAW_API_URL_DEFAULT))
        self.customWsCard = LineEditSettingCard(FIF.LINK, 'MiniPet 协议后端地址', '连接 miniClaw 或其他按 MiniPet 通用协议实现的智能体服务', placeholder=config.CUSTOM_AGENT_WS_DEFAULT, parent=self.agentGroup)
        self.customWsCard.setText(cfg.get('custom_agent_ws_url', config.CUSTOM_AGENT_WS_DEFAULT))
        self.noteCard = PushSettingCard('打开', FIF.MESSAGE, '说明', '只有“使用内置大模型”会记录对话；OpenClaw 需先开启 Responses HTTP API；协议后端接入文档', self.agentGroup)
        self.noteCard.clicked.connect(lambda: QDesktopServices.openUrl(QUrl.fromLocalFile(str(config.ROOT_DIR / 'minipet交互协议设计.md'))))

        for card in [self.backendCard, self.openclawApiCard, self.customWsCard, self.noteCard]:
