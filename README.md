...
- 设置中开启后，鼠标中键作为全局语音输入快捷键
- 第一次按中键：语音球强制弹出并切换到 `typing` 状态（红色波形，区别于语音聊天的绿色）
- 录音期间实时显示 ASR 识别文字，语音球宽度随文字动态扩展
- 第二次按中键：停止录音，识别文字自动注入到当前焦点输入框（模拟键盘输入）
- 录音结束后语音球自动恢复原状态（待机/等待唤醒词/关闭）
- 与语音聊天互斥：语音聊天正在录音/思考/播放期间，中键不触发日常输入
- 录音期间自动暂停唤醒词监听，结束后恢复
...
- 新增 `日常语音输入` 功能：中键开始/停止录音，识别文字自动注入当前输入框
- 语音球新增 `typing` 状态（红色三条波形）用于日常输入，与语音聊天状态视觉区分
- 日常输入与语音聊天互斥，语音聊天进行中中键不触发
- 修复聊天窗口流式回复不实时更新的 bug（`startStream` 早于 `appendMessage` 导致 stream bubble 被销毁）
- 语音球最大宽度从 260 扩展到 420，支持显示更长的识别文字
- 所有主题菜单背景改为不透明纯色，解决深色桌面背景下颜色被稀释显"淡"的问题
- 语音设置页服务开通/在线体验/API 教程改为标题行文字超链接，替代底部独立卡片
- 日常语音输入设置合并到语音页底部"语音输入小助手"分组，删除独立导航入口
